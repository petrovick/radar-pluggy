...
const migration = require('../../../../src/infra/db/migrations/20260907100100-criar-radar-pluggy-calls.cjs') as {
...
describe('contrato: model factory de radar_pluggy_calls vs. migration real', () => {
  const sequelize = createDatabaseConnection(testDatabaseConfig())
  const queryInterface = sequelize.getQueryInterface()

  beforeAll(async () => {
    const tables = await queryInterface.showAllTables()
    if (!tables.includes('radar_pluggy_calls')) {
      try {
  ...
  it('toda coluna da migration real tem uma coluna correspondente no model, e vice-versa', async () => {
    const realColumns = await queryInterface.describeTable('radar_pluggy_calls')
  ...
  it('down da migration desfaz índices e remove a tabela', async () => {
    const testTableName = 'test_down_pluggy_calls'
    const tables = await queryInterface.showAllTables()
    if (tables.includes(testTableName)) {
      await queryInterface.dropTable(testTableName)
    }

    const proxyQI = createProxyQueryInterface(queryInterface, 'radar_pluggy_calls', testTableName)
