...
describe('contrato: model factory de pluggy_connector_investment_transactions vs. migration real', () => {
  ...
  it('toda coluna da migration real tem uma coluna correspondente no model, e vice-versa', async () => {
    const realColumns = await queryInterface.describeTable('pluggy_connector_investment_transactions')
  ...
  it('down da migration desfaz índices e remove a tabela', async () => {
    const testTableName = 'test_down_inv_transactions'
    const tables = await queryInterface.showAllTables()
    if (tables.includes(testTableName)) {
      await queryInterface.dropTable(testTableName)
    }

    const proxyQI = createProxyQueryInterface(queryInterface, 'pluggy_connector_investment_transactions', testTableName)
