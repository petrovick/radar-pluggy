...
const migration = require('../../../../src/infra/db/migrations/20260907100200-criar-radar-pluggy-item-ingestion-leases.cjs') as {
...
describe('contrato: model factory de radar_pluggy_item_ingestion_leases vs. migration real', () => {
  const sequelize = createDatabaseConnection(testDatabaseConfig())
  const queryInterface = sequelize.getQueryInterface()

  beforeAll(async () => {
    const tables = await queryInterface.showAllTables()
    if (!tables.includes('radar_pluggy_item_ingestion_leases')) {
      try {
  ...
  it('toda coluna da migration real tem uma coluna correspondente no model, e vice-versa', async () => {
    const realColumns = await queryInterface.describeTable('radar_pluggy_item_ingestion_leases')
  ...
  it('down da migration desfaz índices e remove a tabela', async () => {
    const testTableName = 'test_down_item_ingestion_leases'
    const tables = await queryInterface.showAllTables()
    if (tables.includes(testTableName)) {
      await queryInterface.dropTable(testTableName)
    }

    const proxyQI = createProxyQueryInterface(queryInterface, 'radar_pluggy_item_ingestion_leases', testTableName)
