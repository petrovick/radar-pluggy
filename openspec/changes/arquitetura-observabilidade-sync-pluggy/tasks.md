...
## 2. Marca d'água por produto (D4, D5)

- [ ] 2.1 Migration: criar `radar_pluggy_product_sync_states` (`item_id`, `product`,
...
- [ ] 2.3 `LoadPluggyHistoryImpl`/`Interactor`: portão passa de `radar_pluggy_history_sync_states`
      (item-level) para `PluggyProductSyncStateRep` por produto (`CASH`, `CUSTODY`); verificar com
      os cenários da spec `pluggy-transaction-history` (dia sem mudança em nenhum produto vs. um
      produto avança sem esperar o outro)
- [ ] 2.4 `SyncPluggyPositionImpl`/`Interactor`: aceitar `executionStatus` `SUCCESS` ou
      `PARTIAL_SUCCESS`; processar `investments`/`loans` de forma independente usando 1.3; recusa de
...
- [ ] 3.1 Migrations: `radar_pluggy_item_observations` e `radar_pluggy_calls` (`item_id` nullable,
      `call_scope` com `PLATFORM_CONFIG`, `trigger` incluindo `CREDENTIAL_REGISTRATION_PRELOAD` e
      `CREDENTIAL_REGISTRATION_VALIDATION`); verificar com `npm run migrate` limpo
- [ ] 3.2 Entities `PluggyItemObservation` (não-regressão de `observedAt`) e `PluggyCall`
      (obrigatórios + enum) e seus repositórios, cada um com transação própria, independente da
      transação de negócio do chamador; verificar com teste de contrato e teste de que a escrita
      sobrevive a um rollback do chamador
...
## 4. Estado observado e identidade do connector (D8, D9)
...
- [ ] 4.2 `PluggyItemStateResolver` (novo, `.scoped()`, cache por escopo de trabalho): `read(itemId)`
      busca via `PluggyItemCredentialResolver` + `PluggyItemsGateway`, persiste
      `radar_pluggy_item_observations` (transação própria) e devolve o snapshot; segunda chamada no
      mesmo escopo não bate na rede; verificar com teste contando chamadas ao client mockado
