...
- [ ] 1.1 `adapters/gateways/pluggy-source-catalog.ts`: tabela única `ProductType` (SDK, ex.
...
- [ ] 2.1 `PluggyItemsGateway`: corrigir a chave de `statusDetail` de `investmentsTransactions` para
...
- [ ] 2.2 `PluggyItem.VALID_STATUSES`: adicionar `WAITING_USER_ACTION` e `MERGING`; verificar com
      teste que `PluggyItem.create`/`reconstitute` aceitam os dois novos valores sem lançar
- [ ] 2.3 Extrair `toSourceState`/`isUsable` (baseado em `isUpdated`, não em código de `warning`,
...
- [ ] 2.4 `PluggyItemsGateway.parseItem`: parsear `data.products` (campo `products` do payload bruto
...
- [ ] 2.5 `PluggyItemsGateway.parseItem`: parsear `data.updatedAt` como campo obrigatório (SDK:
...
- [ ] 2.6 `toVersionAt(item)` (usa 1.1/2.3): `Item.lastUpdatedAt ?? Item.updatedAt` quando
      ...
      nomeado")
- [ ] 2.7 `LoadPluggyHistoryImpl`: trocar a derivação local de `limitedByRateLimit` pela função
...
- [ ] 3.1 `PluggyItemRep.save`: trocar `row.update(...)` incondicional por `UPDATE ... WHERE item_id
      ...
      ordem de conclusão
- [ ] 3.2 `PluggyHistoryCoverageRep.save`: mesma troca, condição por `last_completed_scan_at`/
...
- [ ] 4.1 Migration: criar `radar_pluggy_sync_progress` (`item_id`, `consumer` — valores
...
- [ ] 4.2 Entity `PluggySyncProgress` (create/reconstitute, avanço sem regressão) e
...
- [ ] 4.3 Gate de elegibilidade por fonte: `enabledForItem` (de `itemProducts`, 2.4) MUST ser
...
- [ ] 4.4 `LoadPluggyHistoryImpl`/`Interactor`: portão passa de `radar_pluggy_history_sync_states`
...
- [ ] 4.5 `LoadPluggyHistoryImpl`/`Interactor`: `ACCOUNT_TRANSACTIONS` só avança (D13) quando a
...
- [ ] 4.6 `LoadPluggyHistoryImpl`: varredura de transações deixa de filtrar por
...
- [ ] 4.7 `SyncPluggyPositionImpl`/`Interactor`: aceitar `executionStatus` `SUCCESS` ou
...
- [ ] 4.8 `radar_pluggy_items.last_updated_at`: confirmar (teste de regressão) que só avança quando
...
- [ ] 5.1 `SyncPluggyPositionInteractor.execute`: remover a recusa incondicional de lista vazia de
...
- [ ] 5.2 `savePositionsWithSnapshots`/`saveLoansWithSnapshots`
      ...
      vazia)
- [ ] 5.3 `LoadPluggyHistoryImpl.readCashSources`: mesma reconciliação para `PluggyAccountRep`,
...
- [ ] 5.4 Confirmar que `PluggyPositionSnapshotRep`/`PluggyPositionRawRep`/equivalentes de
...
- [ ] 6.1 `PluggyWebhookEvent`: trocar `isApplicable(): boolean` por `categorize(): 'FULL_INGESTION' |
...
- [ ] 6.2 Migration: `radar_pluggy_credential_items.inactive_at` (nullable); verificar com
      `npm run migrate` limpo
- [ ] 6.3 `webhook-drainer.ts`: para `FULL_INGESTION`, fluxo atual (seção 7); para
      ...
      sem tentar reler")
- [ ] 6.4 Tradução `(status, executionStatus) → connectionStatus`: checar `inactive_at` **antes** de
...
- [ ] 6.5 `PluggyPersonItemResolver.itemIdsFor(personId)`: filtrar `inactive_at IS NULL` — corrige
...
- [ ] 7.1 Migration: `radar_pluggy_item_ingestion_leases` (`item_id` único, `trigger`, `lease_until`,
      `fencing_token` BIGINT, `acquired_at`); verificar com `npm run migrate` limpo
- [ ] 7.2 `PluggyItemIngestionLeaseRep`: `tryAcquire(itemId, trigger, ttlMs)` (incrementa
...
- [ ] 7.3 `infra/worker/pluggy-item-ingestion.ts` (novo — substitui a orquestração ad hoc de
...
- [ ] 7.4 `webhook-drainer.ts`: assinatura passa a `drainPluggyWebhookEvents(container, trigger)` —
...
- [ ] 7.5 `RegisterPluggyCredentialInteractor`/handler: trocar a chamada a
...
- [ ] 7.6 `load-pluggy-history.handler.ts` (rota manual): todo o corpo do handler (aquisição do lease,
...
- [ ] 7.7 `index.ts` (boot): chama `drainPluggyWebhookEvents(container, 'BOOT_RECOVERY')`
...
- [ ] 8.1 Migrations: `radar_pluggy_item_observations` com schema completo (D33 — `id`, `item_id`
...
- [ ] 8.2 Entities `PluggyItemObservation` (não-regressão de `observationStartedAt`, condição `>`
      estrita — D9.1) e `PluggyCall` (obrigatórios + enums de `operation`/`call_scope`/`trigger`/
      `outcome`/`failure_kind`)
- [ ] 8.3 `PluggyCallRecorder` (D31): serviço singleton (registrado no container raiz, mesmo padrão
...
- [ ] 8.4 `infra/tools/call-context.ts` (`AsyncLocalStorage`, `runWithCallContext`/
...
- [ ] 8.5 `PluggyClientGateway`: recebe `PluggyCallRecorder` via DI e repassa para cada
...
- [ ] 8.6 `adapters/gateways/pluggy-call-instrumentation.ts`:
...
- [ ] 8.7 Aplicar `instrumentPluggyClient` (com o `pluggyCallRecorder` resolvido do próprio escopo)
...
- [ ] 8.8 `runWithCallContext` nos pontos de entrada, sempre com `trigger` recebido explicitamente do
      chamador (nunca fixo internamente, D23): `webhook-drainer.ts` (`trigger` recebido por
      parâmetro — seção 7.4/7.7), `pluggy-item-ingestion.ts` (`trigger` recebido do chamador),
      ...
      `radar_pluggy_calls` para cada fluxo, incluindo o teste de 7.7 (`BOOT_RECOVERY` distinto de
      `WEBHOOK` na mesma função de drenagem)

## 9. Estado observado e identidade do connector (D8, D9, D9.1, D18, D19, D26, D33)

- [ ] 9.1 `PluggyItemsGateway.parseItem`: extrair `connector: {id, name, imageUrl, primaryColor,
...
- [ ] 9.2 `PluggyItemStateResolver` (novo, `.scoped()`, cache por escopo de trabalho, usado só por
...
- [ ] 9.3 Trocar `SyncPluggyPositionImpl.readCurrentItemState` e
...
- [ ] 9.4 Migration: colunas de connector em `radar_pluggy_credential_items` (`connector_id`,
...
- [ ] 9.5 `RegisterPluggyCredentialInteractor`/`Impl`: `saveCredentialWithItemLink` passa a receber e
...
- [ ] 9.6 Tradução `(status, executionStatus) → connectionStatus` (seção 6.4 já cobre `inactive_at`
...
- [ ] 10.1 `CheckPluggyCredentialGateway`: novo método lendo `radar_pluggy_item_observations` +
...
- [ ] 10.2 `CheckPluggyCredentialInteractor`/handler: novo shape
      ...
      portfolio)
- [ ] 10.3 Teste ponta a ponta do cadastro: depois de `POST /credentials`, aguardar a ingestão
...
- [ ] 11.1 `pluggy-credentials.api.ts`: `PluggyCredentialStatusResponseDto` ganha `items[]` (com
      `sources`, cada uma com `supportedByConnector`/`enabledForItem`/`isUpdated`/`lastUpdatedAt`);
      `checkStatus()` devolve o objeto inteiro
- [ ] 11.2 `pluggy-credentials.store.ts`: `items` como novo estado; `register()` chama
      `checkStatus()` ao final, antes de devolver sucesso
- [ ] 11.3 `pluggy-portfolio.store.ts` e `pluggy-card-statement.store.ts`: `fetchList()` dispara
      `usePluggyCredentialsStore().checkStatus()` em paralelo
- [ ] 11.4 Componente/composable `PluggyConnectionBanner`: prioridade `DISCONNECTED` >
      ...
      `connectorImageUrl`; verificar com teste de tabela de casos
- [ ] 11.5 `PluggyPortfolioView.vue` e `PluggyCardStatementView.vue`: usar o banner; lista vazia só
      mostra "Nenhuma posição encontrada" quando a fonte relevante está `enabledForItem`/`isUpdated`
      e genuinamente vazia — nunca quando pendente, recusada, ou não habilitada; item `DISCONNECTED`
      não contribui à lista; verificar com teste de view

## 12. `ADR-radar-pluggy-open-finance-consumo-sincronizacao.md`

- [ ] 12.1 Banner de status já adicionado nesta rodada de planejamento (ver topo do arquivo) —
      conferir, na implementação, que nenhuma seção normativa do ADR sobre watermark global,
      `PARTIAL_SUCCESS` da posição, ou ausência de `radar_pluggy_calls` é seguida em vez das specs
      deste change

## 13. Verificação final

- [ ] 13.1 `radar-pluggy`: `npm run lint` (inclui `tsc --noEmit`) e `npm test` verdes em Docker
- [ ] 13.2 `oplab-radar-front`: `npm run lint`, `npm run type-check`, `npm test` verdes
- [ ] 13.3 `openspec validate arquitetura-observabilidade-sync-pluggy --strict` sem erros
...
- [ ] 13.5 Varredura de consistência: nenhuma menção vigente (fora de "alternativa descartada") a
      ...
      `item/updated` como únicos eventos relevantes
