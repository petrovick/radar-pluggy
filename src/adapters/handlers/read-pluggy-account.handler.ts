...
// Único ponto de entrada externo da lista de contas (`GET /accounts`) — só traduz request/response
// HTTP, a leitura mora no interactor resolvido do escopo. `personId` nunca vem do corpo/query — vem
// de `req.personId`, resolvido pelo middleware de autenticação a partir do JWT.
export async function readPluggyAccountHandler(
  ...
  try {
    ...
    const interactor = req.container.resolve('readPluggyAccountInteractor')
    ...
    res.status(500).json({ errorType: 'PLUGGY_ACCOUNT_READ_FAILED' })
  }
}
